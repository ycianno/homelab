...
Deployed on: automation-01 (10.0.0.67)
Ollama host: Mac M4 (10.0.0.72:11434)
Database:    /opt/stacks/life-control-center/data/database.sqlite
...
DEFAULT_CONFIG = {
    "database_path": "/opt/stacks/life-control-center/data/database.sqlite",
    "ollama_url": "http://10.0.0.72:11434",
...
RANKS = [(1, "Initiate"), (8, "Apprentice"), (16, "Journeyman"), (26, "Artisan"), (40, "Master"), (60, "Forgemaster")]
...
def compute_profile(weeks, settings) -> dict:
    ...
    return {
        ...
        "insignias": list((settings.get("insignias") or {}).keys()),
        "trophies": {g: len((settings.get("trophies") or {}).get(g) or {}) for g in ("bronze", "silver", "gold", "platinum")},
    }
...
def detect_milestones(profile, state):
    ...
    # Insignias — silent until recorded once (also silently migrates from the old "badges" key)
    if "insignias" in state:
        new_insignias = [b for b in profile["insignias"] if b not in state["insignias"]]
    else:
        new_insignias = []
    # Trophies — flag any grade whose count rose since the last run
    new_trophies = []
    if "trophies" in state:
        for g in ("bronze", "silver", "gold", "platinum"):
            if profile["trophies"].get(g, 0) > state["trophies"].get(g, 0):
                new_trophies.append(g)
    ...
    return {"leveled_up": leveled_up, "new_insignias": new_insignias,
            "new_trophies": new_trophies, "streak_milestone": streak_milestone}
...
def build_prompt(config, profile, status, milestones, mode, hour, boss):
    ...
    for g in milestones["new_trophies"]:
        mile.append(f"Earned a {g.upper()} trophy.")
    if milestones["new_insignias"]:
        mile.append(f"Unlocked {len(milestones['new_insignias'])} new insignia(s).")
...
TROPHIES: {profile['trophies']['bronze']} bronze, {profile['trophies']['silver']} silver, {profile['trophies']['gold']} gold, {profile['trophies']['platinum']} platinum
...
def fallback_message(profile, status, milestones, mode):
    ...
    for g in milestones["new_trophies"]:
        head.append(f"{g.title()} trophy earned!")
    if milestones["new_insignias"]:
        head.append(f"{len(milestones['new_insignias'])} new insignia(s) unlocked.")
    if mode == "morning":
...
def post_discord(config, profile, status, message, mode, milestones, boss, is_test=False):
    ...
    fields = [
        ...
        {"name": "🏆 Trophies", "value": f"🥉{profile['trophies']['bronze']} 🥈{profile['trophies']['silver']} 🥇{profile['trophies']['gold']} 🏆{profile['trophies']['platinum']}", "inline": True},
    ]
    mile = []
    if milestones["leveled_up"]:
        mile.append(f"⬆️ Reached **Level {profile['level']}**")
    for g in milestones["new_trophies"]:
        emoji = {"bronze": "🥉", "silver": "🥈", "gold": "🥇", "platinum": "🏆"}[g]
        mile.append(f"{emoji} **{g.title()} trophy** earned")
    if milestones["new_insignias"]:
        mile.append(f"🎖️ **{len(milestones['new_insignias'])}** new insignia(s)")
    if milestones["streak_milestone"]:
...
def main():
    parser = argparse.ArgumentParser(description="Hermes — LCC Game Master & Discord Agent")
    ...
    has_milestone = (milestones["leveled_up"] or milestones["new_insignias"]
                     or milestones["new_trophies"] or milestones["streak_milestone"])
    ...
    if args.verbose:
        ...
        print(f"[Hermes] mode={mode} milestones={milestones} insignias={len(profile['insignias'])} trophies={sum(profile['trophies'].values())}")
    ...
    if not args.test:
        ...
        state["insignias"] = profile["insignias"]
        state["trophies"] = profile["trophies"]
        if mode == "morning":
